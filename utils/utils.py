...
def fix_byte_limit(path: str, byte_limit=250):
    # Normalize path separators - handle both absolute and relative paths
    # Try to get relative path, but fall back to absolute if on different drive
    try:
        rel_path = os.path.relpath(path).replace('\\', '/')
    except ValueError:
        # Path is on a different drive, use absolute path instead
